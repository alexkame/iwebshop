...
class Ad
{
	...
	public static function show($position,$goods_cat_id = 0)
	{
		...
		if($positionObject)
		{
			$adList = self::getAdList($positionObject['id'],$goods_cat_id,GetIP::IpMessage(GetIP::clintip())[1]);
...
OEF;
	 ...
	 * @return array
	 */
	public static function getAdList($position,$goods_cat_id = 0,$region_id)
	{
		$now    = date("Y-m-d H:i:s",ITime::getNow());
		$adDB   = new IModel("ad_manage,region");
		return $adDB->query("iwebshop_ad_manage.id = iwebshop_region.ad_id and position_id=1 and region_id=110000 and goods_cat_id = 0 and start_time < '{$now}' AND end_time > '{$now}'");
	}
