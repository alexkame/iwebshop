...
class Market extends IController implements adminAuthorization
{
	...
	function code_edit()
	{
        ...
        $ticketObj = new IModel('jihuoma');

        $dataArray = array(
            'code'      => IFilter::act(IReq::get('code','post')),
            'mianzhi'     => IFilter::act(IReq::get('mianzhi','post')),
        ...
        {
            $ticketObj->add();
            // $this->redirect('code_list');
        }
        $this->redirect('code_edit');
	}

	//展示激活码
	function code_list()
	{
		$jihuo = new IModel('jihuoma');
        $list = $jihuo->sqlquery('select * from iwebshop_jihuoma');
//        $this->data=$list;
//        print_r($list);
        $this->setRenderData(array('list'=>$list));
        $this->redirect('code_list');
	}
	...
	function daijin()
	{
		$jihuoma   = IFilter::act(IReq::get('str'));
        $jihuo = new IModel('jihuoma');
        $arr = $jihuo->getObj("code='$jihuoma'");
        $jihuo->setData(array('status'=>1));
        $where = 'id='.$arr['id'];
        $jihuo->update($where);
        $result = array(
            'mianzhi'=>$arr['mianzhi'],
